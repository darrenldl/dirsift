  ...
  | Git ->
    let sub_dirs =
      try
        Sys.readdir dir
      with
      | _ -> failwith "Failed to read directory"
    in
    Array.mem ".git" sub_dirs
  | Hidden ->
    String.get dir 0 = '.'
  | Not x ->
    not (dir_matches_typ dir x)
...
let run (typs : dir_typ list) (dir : string) =
  ...
  sub_dirs
  |> List.filter (fun dir ->
      List.for_all (dir_matches_typ dir) typs)
...
let typ_arg =
  ...
  in
  let doc =
    "$(docv) is one of git, not-git"
  in
  Arg.(value & opt_all (enum typs) [] & info ["t"; "type"] ~doc ~docv:"TYPE")

let dir_arg =
  Arg.(value & pos 0 dir "." & info [])

let cmd =
  let doc = "Filter directories which satisfy all directory types constraints" in
